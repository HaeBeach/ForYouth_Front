<template>
    ...
    <table>
        ...
        <tr v-for="i in noticeList.length" :key={i}>
            ...
            <td id="column2">{{ noticeList[i - 1].title }}</td>
            <td id="column3">{{ noticeList[i - 1].depNm }}</td>
    ...
    methods: {
        loadTableBody: async function(currentPage) {
            ...
            if (currentPage < 1)
                ...
                return;
            const res = await axios.get(`http://${VUE_APP_BACK_SERVICE_HOST}:${VUE_APP_BACK_SERVICE_PORT}/realty/sh/pageSize/${this.pageSize}/pageNumber/${currentPage}`);
            console.log(res);
            ...
            console.log(this.noticeList[0].title);
        }
    }
}
</script>
